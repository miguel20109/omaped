...
{
    private $admin, $session, $usuarios, $clientes, $prestamos, $cajas, $fallecidos;
    ...
    public function dashboard()
    {
		...
		$data['fallecidos'] = $this->fallecidos->where('estado', '1')->countAllResults();
		
		$data['cementerio1'] = $this->fallecidos->where('estado','1')->where('IDcementerio','1')->countAllResults();
		
		$data['cementerio2'] = $this->fallecidos->where('estado','1')->where('IDcementerio','2')->countAllResults();
		
		$data['cementerio3'] = $this->fallecidos->where('estado','1')->where('IDcementerio','3')->countAllResults();		
		
		$data['porcen1']= round(($data['cementerio1']/$data['fallecidos']) * 100, 0);
		$data['porcen2']= round(($data['cementerio2']/$data['fallecidos']) * 100, 0);;
		$data['porcen3']= round(($data['cementerio3']/$data['fallecidos']) * 100, 0);;
    ...
    public function fallecidosMes($anio)
    {
        $desde = $anio . '-01-01 00:00:00';
        $hasta = $anio . '-12-31 23:59:59';

        $where = "created_at BETWEEN '$desde' AND '$hasta' AND estado = 1";
        $data['total'] = $this->fallecidos->select("
        ...
        SUM(IF(MONTH(created_at) = 12, 1, 0)) AS dic")
            ->where($where)->first();
        //calcular cantidad
		$data['max'] = $this->fallecidos->where($where)->countAllResults();
    ...
    public function createBackup()
    {
        if (verificar('backup', $this->session->permisos)) {
            ...
            $backupFilename = 'RUOS_backup_' . date('dmY_Hi') . '.sql';
            ...
            if (!empty($backupData)) {
                if (file_put_contents($backupPath, $backupData) !== false) {
                    if ($this->crearZip($backupPath)) {
                        ...
                        $filePath = './backup.zip';
                        $fileName = 'RUOS_backup_' . date('dmY_Hi') . '.zip';
